namespace Linn.Stores2.Domain.LinnApps.Tests.RequisitionReportServiceTests
{
    ...
    public class WhenGettingFromStockCostReportWithNoBudget : ContextBase
    {
        ...
        public async Task SetUp()
        {
            ...
            this.part = new Part { PartNumber = "P1", Description = "P1 D", BaseUnitPrice = 123.45678m };
            ...
            var line = new LineWithMoves(
                this.reqNumber,
                1,
                TestData.Transactions.TestTransDefs.StockToAdjust,
                3,
                this.part);
        ...
        public void ShouldReturnReport()
        {
            ...
            this.result.GetGridValue(0, 2).Should().Be(123.45678m);
            this.result.GetGridValue(0, 3).Should().Be(3m);
            this.result.GetGridValue(0, 4).Should().Be(370.37m);
            this.result.GetGridValue(0, 5).Should().Be(-370.37m);
            this.result.GetTotalValue(4).Should().Be(370.37m);
            this.result.GetTotalValue(5).Should().Be(-370.37m);
        }
    }
}
