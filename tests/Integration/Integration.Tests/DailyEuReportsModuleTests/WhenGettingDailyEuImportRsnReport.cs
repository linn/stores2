...
namespace Linn.Service.Integration.Tests.RsnReportsModuleTests
{
    ...
    public class WhenGettingDailyEuImportRsnReport : ContextBase
    {
        [SetUp]
        public void Setup()
        {
            ...
            this.DailyEuReportService.GetDailyEuImportRsnReport(fromDate.ToString("o"), toDate.ToString("o"))
                .Returns(result);

            this.Response = Client.Get(
                $"/stores2/customs/daily/eu/import/rsn/report?fromDate={fromDate.ToString("o")}&toDate={toDate.ToString("o")}",
